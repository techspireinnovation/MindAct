    ...
    public function up(): void
    {
        ...
        if (! Schema::hasColumn($tableNames['permissions'], 'deleted_at')) {
            ...
            if ($this->indexExists($tableNames['permissions'], $oldIndex)) {
                Schema::table($tableNames['permissions'], function (Blueprint $table) use ($oldIndex) {
        ...
        if (! Schema::hasColumn($tableNames['roles'], 'deleted_at')) {
            ...
            if ($this->indexExists($rolesTable, $oldUnique)) {
                Schema::table($rolesTable, function (Blueprint $table) use ($oldUnique) {
                    $table->dropUnique($oldUnique);
                });
            }

            Schema::table($rolesTable, function (Blueprint $table) use ($columnNames) {
    ...
    public function down(): void
    {
        ...
        if (Schema::hasColumn($tableNames['roles'], 'deleted_at')) {
            ...
            if ($this->indexExists($rolesTable, $newUnique)) {
                Schema::table($rolesTable, function (Blueprint $table) use ($newUnique) {
        ...
        if (Schema::hasColumn($tableNames['permissions'], 'deleted_at')) {
            ...
            if ($this->indexExists($permsTable, $newUnique)) {
                Schema::table($permsTable, function (Blueprint $table) use ($newUnique) {
                    $table->dropUnique($newUnique);
                });
            }
