...
class SaleReturnObserver
{
    ...
    public function created(SalesReturn $saleReturn): void
    {
        ...
        $customerName = isset($sale->customer) ? $saleReturn->customer->party_name : $saleReturn->customer_name;
        $ledgerType = isset($sale->customer) ? $saleReturn->customer->ledger_type : 'custom';
        ...
        $voucher = VoucherSummary::firstOrCreate([
            ...
            'branch_id' => null,
            'voucher_number' => "SLVOU-828300{$saleReturn->invoice_number}",
            'particulars' => "Product Sale Returned from {$customerName} - Bill No. {$saleReturn->invoice_number}",
        ...
        switch ($ledgerType) {
            case 'customer':
                ...
                $partyHead = AccountHead::where(['name' => $customerName, 'company_id' => $saleReturn->company_id])->first();
            ...
            case 'both':
                ...
                if (isset($sale->payment['credit']) && $saleReturn->payment['credit'] !== null)
                    $purchaseAccGroups['Accounts Payable (Creditors)'] = ['type' => 'credit', 'valueAmount' => (float) $saleReturn->payment["credit"], 'payment_type' => 'CREDIT'];
                break;

            default:
                $partyAccountGroup = AccountGroup::where(['name' => "Accounts Payable (Creditors)"])->orderBy('code', 'DESC')->first();
                $code = $partyAccountGroup ? (int) $partyAccountGroup->code + 1 : 1;
                $partyHead = AccountHead::firstOrCreate(['name' => $saleReturn->customer_name, 'company_id' => $saleReturn->company_id, 'account_group_id' => $partyAccountGroup->id, 'is_active' => true, 'code' => $code, 'is_primary' => true]);

                if (isset($saleReturn->payment['credit']) && $saleReturn->payment['credit'] !== null)
                    $purchaseAccGroups['Accounts Receivable (Debtors)'] = ['type' => 'debit', 'valueAmount' => (float) $saleReturn->payment["credit"], 'payment_type' => 'CREDIT'];
                break;
        }
        ...
        try {
            foreach ($purchaseAccGroups as $purchaseAccGroupKey => $purchaseAccGroupValue) {
                ...
                if (!$accHead && ($purchaseAccGroupKey == "Accounts Receivable (Debtors)" || $purchaseAccGroupKey == "Accounts Payable (Creditors)")) {
                    ...
                    $accHead = AccountHead::firstOrCreate(['name' => $customerName, 'company_id' => $saleReturn->company_id, 'account_group_id' => $accGroup->id, 'is_active' => true, 'code' => $code, 'is_primary' => true]);
                ...
                if ($purchaseAccGroupValue['valueAmount'] > 0) {
                    VoucherSummaryDetail::create([
                        ...
                        'company_id' => $saleReturn->company_id,
                        'branch_id' => null,
                        'voucher_number' => "SRVOU-828300{$saleReturn->id}",
                        'particulars' => "Product Sale Returned from {$customerName} - Bill No. {$saleReturn->purchase_bill_number}",
            ...
            if (isset($saleReturn->payment['cash']) && $saleReturn->payment['cash'] !== null && $saleReturn->payment['cash'] > 0) {
                $accHead = AccountHead::where(['name' => $customerName, 'company_id' => $saleReturn->company_id])->first();
                VoucherInnerDetail::create([
                    'voucher_summary_id' => $voucher->id,
                    'company_id' => $saleReturn->company_id,
